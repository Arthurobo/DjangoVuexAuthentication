<template>
  <div class="downloads">
    ...
    <div class="bod">
      <h1>Posts section</h1>
      <h2>Posts:</h2>
      <h2 v-for="post in posts" :key="post.id">{{post.name}}</h2>
      <h2></h2>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    created () {
        getAPI.get('/posts/', { headers: { Authorization: `Bearer ${this.$store.state.accessToken}` } }) // proof that your access token is still valid; if not the
